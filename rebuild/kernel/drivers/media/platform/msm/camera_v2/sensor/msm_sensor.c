...
#include "eeprom/msm_eeprom_mid_otp.h"

extern uint8_t get_mid_otp(void);
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (sensor_info->position == BACK_CAMERA_B) {
		if (strcmp(sensor_name, "ov5648_8909") == 0 || strcmp(sensor_name, "ov8856_8909_ofilm") == 0) {
			mid = 0x07;
		} else if (strcmp(sensor_name, "ov5648_8909_sunwin") == 0 || strcmp(sensor_name, "ov8856_8909_sunwin") == 0) {
			mid = 0x01;
		} else {
			return -ENODEV;
		}
		...
		mid_otp = get_mid_otp();
		CDBG("mid: 0x%x, mid_otp: 0x%x\n", mid, mid_otp);
		if (mid != mid_otp) {
			pr_err("msm_sensor_match_id mid doesnot match\n");
			return -ENODEV;
		}
	}
