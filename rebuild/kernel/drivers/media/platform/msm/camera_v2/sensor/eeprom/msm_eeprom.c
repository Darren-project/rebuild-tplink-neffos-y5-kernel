...
#include "msm_eeprom_mid_otp.h"

static uint8_t mid_otp = 250;
...
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
	for (i = 0; i < 3; i++) {
		mid = e_ctrl->cal_data.mapdata[grp_off[i]];
		flag = mid & 0x80;
		if (!flag) {
			mid_otp = mid & 0x7f;
			CDBG("read mid_otp success!\n");
			break;
		}
	}
	if (i == 3) {
		CDBG("read mid_otp failed!\n");
	}
#endif

#ifdef CONFIG_TPLINK_PRODUCT_TP802
	flag = e_ctrl->cal_data.mapdata[0] & 0xFC;
	/* mid store in group 1 */
	if ((flag&0xC0) == 0x40){
		mid_otp = e_ctrl->cal_data.mapdata[1];
	/* mid store in group 2 */
	}else if((flag&0x30) == 0x10){
		mid_otp = e_ctrl->cal_data.mapdata[9];
	}
	CDBG("read mid_otp success, mid is 0x%x !\n", mid_otp);
#endif
	e_ctrl->is_supported |= msm_eeprom_match_crc(&e_ctrl->cal_data);
