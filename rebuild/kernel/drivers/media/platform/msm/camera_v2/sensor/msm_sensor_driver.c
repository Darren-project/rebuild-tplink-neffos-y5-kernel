...
/* [liguanxiong start] create camera on_board_info interface file (1/2) */

/* each member pair with the the index of array "support_camera_modules"
	defined below in order */
enum {
	MODULE_OV2680_CHT852B = 0,
	MODULE_OV2680_SKUHF,
	MODULE_SP2509,
	MODULE_OV5648_8909,
	MODULE_OV5648_8909_SUNWIN,
	MODULE_OV8856_8909_OFILM,
	MODULE_OV8856_8909_SUNWIN,

} front_camera_module_current, back_camera_module_current;
...
struct camera_module support_camera_modules[] = {
	[MODULE_OV2680_CHT852B]     = {"ov2680", "Sunwingroup", "1.0.0"},
	[MODULE_OV2680_SKUHF]       = {"ov2680", "o-film",      "1.0.0"},
	[MODULE_SP2509]             = {"sp2509", "Sunwingroup", "1.0.0"},
	[MODULE_OV5648_8909]        = {"ov5648", "o-film",      "1.0.0"},
	[MODULE_OV5648_8909_SUNWIN] = {"ov5648", "Sunwingroup", "1.0.0"},
	[MODULE_OV8856_8909_OFILM]  = {"ov8856", "o-film",      "1.0.0"},
	[MODULE_OV8856_8909_SUNWIN] = {"ov8856", "Sunwingroup", "1.0.0"},
};

/* called when user space read the front_camera_board_info interface file */
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	*  sensor control structure */
	...
	/* [ liguanxiong start ] create camera on_board_info interface file (2/2) */

	/* CAMERA MODULE: OV2680_CHT852B */
	if (strncmp(slave_info->sensor_name, "ov2680_cht852b", strlen("ov2680_cht852b"))
				== 0) {
		front_camera_module_current = MODULE_OV2680_CHT852B;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_front_camera.attr);
	}

	/* CAMERA MODULE: OV2680_SKUHF */
	if (strncmp(slave_info->sensor_name, "ov2680_skuhf", strlen("ov2680_skuhf"))
				== 0) {
		front_camera_module_current = MODULE_OV2680_SKUHF;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_front_camera.attr);
	}

	/* CAMERA MODULE: SP2509 */
	if (strncmp(slave_info->sensor_name, "sp2509", strlen("sp2509"))
				== 0) {
		front_camera_module_current = MODULE_SP2509;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_front_camera.attr);
	}

	/* CAMERA MODULE: OV5648_8909 */
	if (strncmp(slave_info->sensor_name, "ov5648_8909", strlen("ov5648_8909"))
				== 0) {
		back_camera_module_current = MODULE_OV5648_8909;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_back_camera.attr);
	}

	/* CAMERA MODULE: OV5648_8909_SUNWIN */
	if (strncmp(slave_info->sensor_name, "ov5648_8909_sunwin", strlen("ov5648_8909_sunwin"))
				== 0) {
		back_camera_module_current = MODULE_OV5648_8909_SUNWIN;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_back_camera.attr);
	}

	/* CAMERA MODULE: OV8856_8909_OFILM */
	if (strncmp(slave_info->sensor_name, "ov8856_8909_ofilm", strlen("ov8856_8909_ofilm"))
				== 0) {
		back_camera_module_current = MODULE_OV8856_8909_OFILM;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_back_camera.attr);
	}

	/* CAMERA MODULE: OV8856_8909_SUNWIN */
	if (strncmp(slave_info->sensor_name, "ov8856_8909_sunwin", strlen("ov8856_8909_sunwin"))
				== 0) {
		back_camera_module_current = MODULE_OV8856_8909_SUNWIN;
		rc = sysfs_create_file(kernel_kobj, &dev_attr_back_camera.attr);
	}
...
static void __exit msm_sensor_driver_exit(void)
{
	...
	/* [liguanxiong start] release camera on_board_info interface file */
