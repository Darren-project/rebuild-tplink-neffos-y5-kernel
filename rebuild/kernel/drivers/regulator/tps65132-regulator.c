...
static int tps65132_regulator_enable(struct regulator_dev *rdev)
{
	...
	/* [jiangfuqiang add] support restore last voltage */
#ifdef CONFIG_TPLINK_PRODUCT_TP802
	if (vreg->vol_set_postpone || vreg->vol_auto_restore) {
#else
		...
		int min_uV, int max_uV, unsigned *selector)
{
	...
	if (!vreg->is_enabled) {
		...
		if (rc) {
			...
			/* [jiangfuqiang start] remember the last vol */
#ifdef CONFIG_TPLINK_PRODUCT_TP802
			if (vreg->vol_auto_restore) {
				vreg->vol_set_val = val;
			}
#endif
			/* [jiangfuqiang end] */
				...
				struct i2c_client *client)
{
	...
	for (i = 0; i < chip->num_regulators; i++) {
		...
		/* [jiangfuqiang start] init vol_auto_restore as true */
#ifdef CONFIG_TPLINK_PRODUCT_TP802
		chip->vreg[i].vol_auto_restore = true;
#endif
		/* [jiangfuqiang end] */
	}
