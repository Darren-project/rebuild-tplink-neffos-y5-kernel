...
/*[lidebiao start]*/
#define CHG_ENABLE_DLY_MS			5000
#define BATT_DETECT_DLY_MS			1000
#define COLD_TEMP_DEVIATION                     100
#define BATT_DETECT_COUNT_MAX			10
/*[lidebiao end]*/
 ...
 * @bat_is_cold:		indicates that battery is cold
 * @bat_is_hot:		        indicates that battery is hot
 ...
 * @cfg_hot_bat_decidegc:	hot battery temperature in degree Celsius
 * @cfg_cold_bat_decidegc:	cold battery temperature in degree Celsius
...
struct qpnp_lbc_chip {
	...
	/*[lidebiao start]*/
	struct delayed_work		temp_detection_work;
	struct delayed_work		bat_detection_work;
	/*[lidebiao end]*/
...
/*[lidebiao start]*/
static int qpnp_lbc_charger_enable_hot_or_cold(struct qpnp_lbc_chip *chip)
{
	int rc;
	if (chip->bat_is_cold || chip->bat_is_hot) {
                rc = qpnp_lbc_charger_enable(chip, THERMAL, 0);
		if (rc < 0)
		        pr_err("failed to set disable charging rc = %d\n", rc);
        } else {
...
static void qpnp_lbc_set_appropriate_current(struct qpnp_lbc_chip *chip)
{
	...
	if ( (chip->bat_is_cool && chip->cfg_cool_bat_chg_ma) || chip->bat_is_cold)
		chg_current = min(chg_current, chip->cfg_cool_bat_chg_ma);
	if ( (chip->bat_is_warm && chip->cfg_warm_bat_chg_ma) || chip->bat_is_hot)
			...
			enum power_supply_property psp, int temp_degc)
{
	...
	case POWER_SUPPLY_PROP_COOL_TEMP:
		if (temp_degc >= (chip->cfg_warm_bat_decidegc - HYSTERISIS_DECIDEGC) ||
			temp_degc <= (chip->cfg_cold_bat_decidegc + HYSTERISIS_DECIDEGC) ) {
			pr_err("Can't set cool %d higher than warm-20 %d or lower than cold+20 %d\n",
					temp_degc,
					chip->cfg_warm_bat_decidegc - HYSTERISIS_DECIDEGC,
                                        chip->cfg_cold_bat_decidegc + HYSTERISIS_DECIDEGC);
			rc = -EINVAL;
			goto mutex_unlock;
		}

                if ( !(chip->bat_is_hot || chip->bat_is_cool || chip->bat_is_cold || chip->bat_is_warm) ) {
	...
	case POWER_SUPPLY_PROP_WARM_TEMP:
		if (temp_degc <= (chip->cfg_cool_bat_decidegc + HYSTERISIS_DECIDEGC) ||
			temp_degc >= (chip->cfg_hot_bat_decidegc - HYSTERISIS_DECIDEGC) ) {
			pr_err("Can't set warm %d lower than cool %d or higher than hot %d\n",
					temp_degc,
...
static void qpnp_lbc_jeita_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (state == ADC_TM_WARM_STATE) {
			...
			schedule_delayed_work(&chip->temp_detection_work,
				msecs_to_jiffies(CHG_ENABLE_DLY_MS));
        ...
        if (chip->bat_is_cold ^ bat_cold || chip->bat_is_hot ^ bat_hot || chip->bat_is_cool ^ bat_cool || chip->bat_is_warm ^ bat_warm) {
        	spin_lock_irqsave(&chip->ibat_change_lock, flags);
		...
		qpnp_lbc_set_appropriate_current(chip);
		qpnp_lbc_charger_enable_hot_or_cold(chip);
		spin_unlock_irqrestore(&chip->ibat_change_lock, flags);
        }

	pr_debug("bat_is_warm = %d, bat_is_cool = %d, bat_is_hot = %d, bat_is_cold = %d",
			chip->bat_is_warm, chip->bat_is_cool, chip->bat_is_hot, chip->bat_is_cold);
	pr_debug("adc_param.low_temp = %d, adc_param.high_temp = %d\n", chip->adc_param.low_temp, chip->adc_param.high_temp);
        /* [houjihai start] noti userspace when tm exceeds threshold */
        if ((temp <= chip->cfg_cold_bat_decidegc) || (temp >= chip->cfg_hot_bat_decidegc)) {
            pr_debug("power supply changed batt_psy temp = %d\n", temp);
	    power_supply_changed(&chip->batt_psy);
        }
        /* [houjihai end] */
	if (qpnp_adc_tm_channel_measure(chip->adc_tm_dev, &chip->adc_param))
...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
	...
	OF_PROP_READ(chip, cfg_cold_bat_decidegc, "cold-bat-decidegc", rc, 1);
        chip->cfg_cold_bat_decidegc = chip->cfg_cold_bat_decidegc - COLD_TEMP_DEVIATION;
	/*[lidebiao end]*/
	...
	pr_debug("vddmax-mv=%d, vddsafe-mv=%d, vinmin-mv=%d, ibatsafe-ma=$=%d\n",
			...
			chip->cfg_min_voltage_mv,
			chip->cfg_safe_current);
	pr_debug("warm-bat-decidegc=%d, cool-bat-decidegc=%d, hot-bat-decidegc=%d, cold-bat-decidegc=%d, batt-hot-percentage=%d, batt-cold-percentage=%d\n",
			chip->cfg_warm_bat_decidegc,
			chip->cfg_cool_bat_decidegc,
			chip->cfg_hot_bat_decidegc,
...
static irqreturn_t qpnp_lbc_batt_pres_irq_handler(int irq, void *_chip)
{
	...
	/*[lidebiao start]*/
	if (batt_present == 0 ) {
		schedule_delayed_work(&chip->bat_detection_work,
			msecs_to_jiffies(BATT_DETECT_DLY_MS));
	}
	/*[lidebiao end]*/
...
/*[lidebiao start]*/
static void qpnp_lbc_temp_detection_work(struct work_struct *work)
{
	int rc, temp = 0;
	unsigned long flags;
	struct delayed_work *dwork = to_delayed_work(work);
	...
	if (temp >= chip->cfg_hot_bat_decidegc - HYSTERISIS_DECIDEGC) {
		schedule_delayed_work(&chip->temp_detection_work,
				msecs_to_jiffies(CHG_ENABLE_DLY_MS));
	} else if (temp <= chip->cfg_cold_bat_decidegc + HYSTERISIS_DECIDEGC) {
		schedule_delayed_work(&chip->temp_detection_work,
				msecs_to_jiffies(CHG_ENABLE_DLY_MS));
	} else {
		if (chip->bat_is_hot == true) {
			chip->bat_is_hot = false;
			chip->bat_is_warm = true;
			chip->adc_param.low_temp = chip->cfg_warm_bat_decidegc - HYSTERISIS_DECIDEGC;
			chip->adc_param.high_temp = chip->cfg_hot_bat_decidegc;
			chip->adc_param.state_request = ADC_TM_HIGH_LOW_THR_ENABLE;
		} else if (chip->bat_is_cold == true) {
...
/*[lidebiao start]*/
static void qpnp_lbc_bat_detection_work(struct work_struct *work)
{
	int bat_present;
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_lbc_chip *chip = container_of(dwork,
	...
	bat_present=qpnp_lbc_is_batt_present(chip);
	pr_debug("bat_present = %d\n", bat_present);
	if (bat_present == 0) {
		chip->cfg_detection_count=chip->cfg_detection_count + 1;
		printk("lidebiao chip->cfg_detection_count = %d\n", chip->cfg_detection_count);
		if ( chip->cfg_detection_count >= BATT_DETECT_COUNT_MAX) {
			kernel_power_off();
		}
		schedule_delayed_work(&chip->bat_detection_work,
			msecs_to_jiffies(BATT_DETECT_DLY_MS));
	} else if ( bat_present == 1 ) {
		chip->cfg_detection_count = 0;
	}
}
/*[lidebiao end]*/
...
static int qpnp_lbc_main_probe(struct spmi_device *spmi)
{
	...
	/*[lidebiao start]*/
	INIT_DELAYED_WORK(&chip->temp_detection_work,
			qpnp_lbc_temp_detection_work);
	INIT_DELAYED_WORK(&chip->bat_detection_work,
			qpnp_lbc_bat_detection_work);
	/*[lidebiao end]*/
...
static int qpnp_lbc_remove(struct spmi_device *spmi)
{
	...
	/*[lidebiao start]*/
	cancel_delayed_work_sync(&chip->temp_detection_work);
	cancel_delayed_work_sync(&chip->bat_detection_work);
	/*[lidebiao end]*/
