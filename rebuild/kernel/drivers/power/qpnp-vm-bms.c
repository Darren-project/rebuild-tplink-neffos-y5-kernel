...
/*
 * [Yang Yubin start] Add variables for saving the parameters that used by
 * the soc calculated process. These variables would be restored when the
 * dropped battery is inserted.
 */
static int is_battery_drop;
static int current_before_drop;
static int ocv_uv_before_drop;
static int batt_temp_before_drop;  /* battery temperature */
/* [Yang Yubin end] */
...
/*
 * [Yang Yubin start] Add functions to save and restore the parameters that
 * used in the calculational process of soc.
 */
static void save_soc_calculated_parameters(struct qpnp_bms_chip *chip, int temp)
{
	current_before_drop = chip->current_now;
	ocv_uv_before_drop = chip->last_ocv_uv;
	batt_temp_before_drop = temp;
}

static void restore_soc_calculated_parameters(struct qpnp_bms_chip *chip,
		int *temp)
{
	chip->current_now = current_before_drop;
	chip->last_ocv_uv = ocv_uv_before_drop;
	*temp = batt_temp_before_drop;
}
/* [Yang Yubin end] */
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			/*
			 * [Yang Yubin start] Save the parameters when the
			 * battery is present and restore the parameters
			 * when battery is inserted after dropping
			 */
			if (1 == is_battery_drop) {
				is_battery_drop = 0;
				restore_soc_calculated_parameters(chip, &batt_temp);

			} else {
				save_soc_calculated_parameters(chip, batt_temp);
			}
			/* [Yang Yubin end] */
...
{
	/*
	 * [Yang Yubin start] If the battery drops, return its's
	 * original soc, but not the DEFAULT CAPACITY
	 */
	if (1 == is_battery_drop) {
	...
	if (chip->battery_present != present) {
		...
		if (chip->battery_present != -EINVAL) {
			if (present) {
				...
				/*
				 * [Yang Yubin start] Set a flag variable that
				 * indicates the battery has dropped.
				 */
				is_battery_drop = 1;
				/* [Yang Yubin end] */
