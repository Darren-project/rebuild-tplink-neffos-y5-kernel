...
#ifdef CONFIG_TPLINK_PRODUCT_TP802
	/* get regulator */
	ctrl_pdata->pos_reg = NULL;
	ctrl_pdata->neg_reg = NULL;
	ctrl_pdata->pos_reg = devm_regulator_get(&pdev->dev, "pos");
	rc_tmp =  PTR_RET(ctrl_pdata->pos_reg);
	if (rc_tmp) {
		//DEV_ERR("get pos regulator failed!\n");
		/* try to get again */
		ctrl_pdata->pos_reg = NULL;
		ctrl_pdata->pos_reg = devm_regulator_get(&pdev->dev, "tps65132-pos");
		rc_tmp =  PTR_RET(ctrl_pdata->pos_reg);
		if (rc_tmp) {
			DEV_ERR("get pos regulator failed again!\n");
		} else {
			//DEV_ERR("get pos regulator again, succeed!\n");
		}
	}

	ctrl_pdata->neg_reg = devm_regulator_get(&pdev->dev, "neg");
	rc_tmp =  PTR_RET(ctrl_pdata->neg_reg);
	if (rc_tmp) {
		//DEV_ERR("get neg regulator failed!\n");
		/* try to get again */
		ctrl_pdata->neg_reg = NULL;
		ctrl_pdata->neg_reg = devm_regulator_get(&pdev->dev, "tps65132-neg");
		rc_tmp =  PTR_RET(ctrl_pdata->neg_reg);
		if (rc_tmp) {
			DEV_ERR("get neg regulator failed again!\n");
		} else {
			//DEV_ERR("get neg regulator again, succeed!\n");
		}
	}
	/* set voltage but not enable */
	if (regulator_set_voltage(ctrl_pdata->pos_reg, 5500000, 5500000) < 0) {
		DEV_ERR("error, set pos regulator failed!\n");
	} else {
		//DEV_ERR("set valtage 5.0v succeed!\n");
	}
	if (regulator_set_voltage(ctrl_pdata->neg_reg, 5500000, 5500000) < 0) {
		DEV_ERR("error, set neg regulator failed!\n");
	} else {
		//DEV_ERR("set valtage 5.0v succeed!\n");
	}

	/* checkout whether enable */
	if (regulator_is_enabled(ctrl_pdata->pos_reg) > 0) {
		DEV_ERR("error, pos regulator is enable!\n");
	}
	if (regulator_is_enabled(ctrl_pdata->neg_reg) > 0) {
		DEV_ERR("error, neg regulator is enable!\n");
	}
#endif
	/* jiangfuqiang end */
	return rc;
