...
/* [linchao start] add for multi LCDs */
#define BOOYI_LCD_DELAY (150)
/* [linchao end] */
				      ...
				      enum led_brightness value)
{
	...
	int bl_lvl;
//#ifdef CONFIG_TPLINK_PRODUCT_TP801
//	unsigned char *panel_name = "otm8019a";
//#endif
...
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
	/* [linchao start] add to delay booyi LCD */
	//if ((strstr(mfd->panel_info->panel_name, panel_name)) && (0 == mfd->bl_level)) {
	if(0 == mfd->bl_level) {
...
static ssize_t mdss_fb_get_hw_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = fbi->par;
...
static DEVICE_ATTR(hw_info, S_IRUGO, mdss_fb_get_hw_info, NULL);
...
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
	/* [linchao start] Error to light up bl, ignore. Just light up by __mdss_fb_display_thread when in recovery mode */
	if (1 == recovery_state) {
...
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
	if (mdss_panel_is_power_off(cur_power_state) && (1 != charge_state)) {
#else
