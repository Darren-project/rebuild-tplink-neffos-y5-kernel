...
/* [linchao start] Add for ILI9881C */
/* ILI9881C panel specific status variables */
#define ILI9881C_BUF_1_STATUS 0x03
#define ILI9881C_BUF_2_STATUS 0x04
#define ILI9881C_MAX_ERR_CNT 1

#define HX8394F_BUF_1_STATUS 0x73
#define HX8394F_BUF_2_STATUS 0x04
#define HX8394F_MAX_ERR_CNT 1
/* [linchao end] */

#define MIN_REFRESH_RATE 48
#define DEFAULT_MDP_TRANSFER_TIME 14000

/* [linchao start] To delay booyi LCD */
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
#define BOOYI_LCD_DELAY (70)
extern int charge_state;
extern int recovery_state;
#endif
/* [linchao end] */
...
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
	/* [linchao start] Delay to light when unblank in recovery or charge mode */
	if (((1 == recovery_state) || (1 == charge_state)) && (0 == level_pre) && (level != 0)) {
		level_pre = level;
		mdelay(BOOYI_LCD_DELAY);
	}
	/* [linchao end] */
#endif
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		/* [jiangfuqiang start] */
#ifdef CONFIG_TPLINK_PRODUCT_TP802
		/* do enable the pos */
		if (0 != regulator_enable(ctrl_pdata->pos_reg)) {
			DEV_ERR("enable pos regulator failed!\n");
		} else {
			//DEV_ERR("enable pos regulator succeed!\n");
		}
		/* must delay >1 ms */
		usleep(1500);
		/* do enable the reg */
		if (0 != regulator_enable(ctrl_pdata->neg_reg)) {
			DEV_ERR("enable neg regulator failed!\n");
		} else {
			//DEV_ERR("enable neg regulator succeed!\n");
		}
		/* delay at least 5ms */
		usleep(7000);
#endif
		...
		/* [jiangfuqiang start] do disable the pos and neg */
#ifdef CONFIG_TPLINK_PRODUCT_TP802
		/* delay at least 1ms */
		usleep(2500);
		/* must disable neg first for hx8394f */
		/* for ili9881c, it is no matter which one first to disable */
		if (0 != regulator_disable(ctrl_pdata->neg_reg)) {
			DEV_ERR("disable neg regulator failed!\n");
		} else {
			//DEV_ERR("disable neg regulator succeed!\n");
		}
		/* delay 0.5 ms */
		usleep(500);
		/* disable pos */
		if (0 != regulator_disable(ctrl_pdata->pos_reg)) {
			DEV_ERR("disable pos regulator failed!\n");
		} else {
			//DEV_ERR("disable pos regulator succeed!\n");
		}
#endif
		/* [jiangfuqiang end] */
	}
...
{
	pr_debug("%s: status_buf.data[0]:%x\n", __func__, \
            (unsigned int)(ctrl_pdata->status_buf.data[0]));
...
/* [linchao start] Add function for ili9881c */
static int mdss_dsi_ili9881c_read_status(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	pr_debug("%s: status_buf.data[0]:%x\tdata[1]:%x\tdata[2]:%x\n", __func__, \
            (unsigned int)(ctrl_pdata->status_buf.data[0]), \
            (unsigned int)(ctrl_pdata->status_buf.data[1]), \
            (unsigned int)(ctrl_pdata->status_buf.data[2]));
	if (ctrl_pdata->status_buf.data[0] !=
					ctrl_pdata->status_value) {
		ctrl_pdata->status_error_count = 0;
		pr_err("%s: Read back value from panel is incorrect\n",
		...
		if ((ctrl_pdata->status_buf.data[1] == ILI9881C_BUF_1_STATUS) &&
				(ctrl_pdata->status_buf.data[2] == ILI9881C_BUF_2_STATUS))
				ctrl_pdata->status_error_count = 0;
			else
				ctrl_pdata->status_error_count++;
			if (ctrl_pdata->status_error_count >=
					ILI9881C_MAX_ERR_CNT) {
				ctrl_pdata->status_error_count = 0;
				printk("%s: Read value bad. Error_cnt = %i\n",
					 __func__,
					ctrl_pdata->status_error_count);
				return -EINVAL;
			}
...
static int mdss_dsi_hx8394f_read_status(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (ctrl_pdata->status_buf.data[0] !=
		...
		if ((ctrl_pdata->status_buf.data[1] == HX8394F_BUF_1_STATUS) &&
				(ctrl_pdata->status_buf.data[2] == HX8394F_BUF_2_STATUS))
				ctrl_pdata->status_error_count = 0;
			else
				ctrl_pdata->status_error_count++;
			if (ctrl_pdata->status_error_count >=
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (!rc) {
		...
		} else if (!strcmp(data, "reg_read_ili9881c")) {
			ctrl_pdata->status_mode = ESD_REG_ILI9881C;
			ctrl_pdata->status_error_count = 0;
			ctrl_pdata->status_cmds_rlen = 8;
			ctrl_pdata->check_read_status =
						mdss_dsi_ili9881c_read_status;
