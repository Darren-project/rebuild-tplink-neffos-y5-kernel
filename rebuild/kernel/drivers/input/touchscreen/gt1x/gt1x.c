...
/* [jiangfuqiang start] */
#define PINCTRL_STATE_RST_PULL_DOWN       "gt1x_reset_suspend"  /*pull down*/
#define PINCTRL_STATE_RST_PULL_UP         "gt1x_reset_active"   /*pull up*/
#define PINCTRL_STATE_INT_PULL_DOWN       "gt1x_int_suspend"    /*pull down*/
#define PINCTRL_STATE_INT_PULL_UP         "gt1x_int_active"     /*pull up*/

struct gt1x_pinctrl_data {
    struct pinctrl *ts_pinctrl;
    struct pinctrl_state *pinctrl_state_rst_pull_down;
    struct pinctrl_state *pinctrl_state_rst_pull_up;
    struct pinctrl_state *pinctrl_state_int_pull_down;
    struct pinctrl_state *pinctrl_state_int_pull_up;
};

struct gt1x_pinctrl_data pinctrl_data = {NULL, NULL, NULL, NULL};
/* [jiangfuqiang end] */
...
static s32 gt1x_request_io_port(void)
{
	...
	if (ret < 0) {
        ...
        /* [jiangfuqiang start] modify to pull down and output 0 */
...
/* [jiangfuqiang start] */
static int gt1x_pinctrl_lookup_state(struct pinctrl *ts_pinctrl,
        struct pinctrl_state **pp_state, const char* state_name)
{
    *pp_state = pinctrl_lookup_state(ts_pinctrl, state_name);
    if (IS_ERR_OR_NULL(*pp_state)) {
        int retval = PTR_ERR(*pp_state);
        GTP_ERROR("Can't lookup %s pinstate %d\n", state_name, retval);
        return -1;
    }
    return 0;
}
static int gt1x_pinctrl_init(struct i2c_client *client, struct gt1x_pinctrl_data *pl_data)
{
    int retval;

    /* Get pinctrl if target uses pinctrl */
    pl_data->ts_pinctrl = devm_pinctrl_get(&(client->dev));
    if (IS_ERR_OR_NULL(pl_data->ts_pinctrl)) {
        retval = PTR_ERR(pl_data->ts_pinctrl);
        dev_dbg(&client->dev, "Target does not use pinctrl %d\n", retval);
        GTP_ERROR("Target does not use pinctrl %d\n", retval);
        goto err_pinctrl_get;
    }

    if (gt1x_pinctrl_lookup_state(pl_data->ts_pinctrl,
                &pl_data->pinctrl_state_rst_pull_down, PINCTRL_STATE_RST_PULL_DOWN)) {
        GTP_ERROR("Can't lookup %s pinstate %d\n", PINCTRL_STATE_RST_PULL_DOWN, retval);
        goto err_pinctrl_lookup;
    }
    if (gt1x_pinctrl_lookup_state(pl_data->ts_pinctrl,
                &pl_data->pinctrl_state_rst_pull_up, PINCTRL_STATE_RST_PULL_UP)) {
        GTP_ERROR("Can't lookup %s pinstate %d\n", PINCTRL_STATE_RST_PULL_UP, retval);
        goto err_pinctrl_lookup;
    }
    if (gt1x_pinctrl_lookup_state(pl_data->ts_pinctrl,
                &pl_data->pinctrl_state_int_pull_down, PINCTRL_STATE_INT_PULL_DOWN)) {
        GTP_ERROR("Can't lookup %s pinstate %d\n", PINCTRL_STATE_INT_PULL_UP, retval);
        goto err_pinctrl_lookup;
    }
    if (gt1x_pinctrl_lookup_state(pl_data->ts_pinctrl,
                &pl_data->pinctrl_state_int_pull_up, PINCTRL_STATE_INT_PULL_UP)) {
        GTP_ERROR("Can't lookup %s pinstate %d\n", PINCTRL_STATE_INT_PULL_UP, retval);
        goto err_pinctrl_lookup;
    }
    return 0;
...
err_pinctrl_lookup:
    devm_pinctrl_put(pl_data->ts_pinctrl);
err_pinctrl_get:
    pl_data->ts_pinctrl = NULL;
    return retval;
}

int __gt1x_rst_int_select_state(struct pinctrl_state *pstate, const char *param_name)
{
    int ret = -1;
    GTP_DEBUG("in set pin state %s !\n", param_name);

    if (NULL == pstate) {
        return ret;
    }
    if (pinctrl_data.ts_pinctrl) {
        ret = pinctrl_select_state(pinctrl_data.ts_pinctrl, pstate);
        if (ret < 0) {
            GTP_ERROR("set pin state %s failed!\n", param_name);
        }
    }
    return ret;
}
#define gt1x_rst_int_select_state(pstate) __gt1x_rst_int_select_state(pstate, #pstate)

int gt1x_set_int_pull_down(void)
{
    return gt1x_rst_int_select_state(pinctrl_data.pinctrl_state_int_pull_down);
}
int gt1x_set_int_pull_up(void)
{
    return gt1x_rst_int_select_state(pinctrl_data.pinctrl_state_int_pull_up);
}
int gt1x_set_rst_pull_down(void)
{
    return gt1x_rst_int_select_state(pinctrl_data.pinctrl_state_rst_pull_down);
}
int gt1x_set_rst_pull_up(void)
{
    return gt1x_rst_int_select_state(pinctrl_data.pinctrl_state_rst_pull_up);
}
/* [jiangfuqiang end] */
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* [jiangfuqiang start] pinctrl init */
    ret = gt1x_pinctrl_init(gt1x_i2c_client, &pinctrl_data);
    if (ret != 0 || pinctrl_data.ts_pinctrl == NULL) {
        GTP_ERROR("GTP pinctrl state init failed.");
    }
    /* [jiangfuqiang end] */
