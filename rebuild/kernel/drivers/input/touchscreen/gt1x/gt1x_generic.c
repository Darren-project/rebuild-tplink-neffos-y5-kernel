...
/* [jiangfuqiang start] */
static ssize_t gt1x_hw_info_read_proc(struct file *, char __user *, size_t, loff_t *);

static struct proc_dir_entry *gt1x_hw_info_proc_entry = NULL;
static const struct file_operations gt1x_hw_info_fops = {
	.owner = THIS_MODULE,
	.read = gt1x_hw_info_read_proc,
	.write = NULL,
};

static s32 gt1x_init_hw_info_node(void)
{
	gt1x_hw_info_proc_entry = proc_create(GT1X_HW_INFO_PROC_FILE, 0666, NULL, &gt1x_hw_info_fops);
	if (gt1x_hw_info_proc_entry == NULL) {
		GTP_ERROR("Create proc entry /proc/%s FAILED!", GT1X_HW_INFO_PROC_FILE);
		return -1;
	}
	GTP_INFO("Created proc entry /proc/%s.", GT1X_HW_INFO_PROC_FILE);
	return 0;
}

static ssize_t gt1x_hw_info_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	unsigned char gt1x_ic[3][GT1X_HW_INFO_MAX] = {{"djn"}, {"booyi"}, ("truly")};
	unsigned char gt1x_type[3][GT1X_HW_INFO_MAX] = {{"GT5668"}, {"GT5668"}, {"GT5688"}};
	char temp_str_buf[128] = {0}; /* Attention: make sure the buffer is big enough */
	char *ptr = temp_str_buf;
	int sensor_id = 0;
	...
	ptr += sprintf(ptr, "type:\t%s\n", &gt1x_type[sensor_id][0]);
	ptr += sprintf(ptr, "vendor:\t%s\n", &gt1x_ic[sensor_id][0]);
	ptr += sprintf(ptr, "driver_version:\t%s\n", GTP_DRIVER_VERSION);

	if (0 != copy_to_user(page, temp_str_buf, (ptr-temp_str_buf))) {
		GTP_ERROR("copy hw_info to user space failed!\n");
	}

	*ppos += ptr - temp_str_buf;
	return (ptr - temp_str_buf);
}
/* [jiangfuqiang end] */
...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	...
	if (0 != copy_to_user(page, temp_str_buf, (ptr-temp_str_buf))) {
		GTP_ERROR("copy debug_info to user space failed!\n");
	}
...
s32 gt1x_init_panel(void)
{
	...
	printk("Config group%d used, length:%d", sensor_id, cfg_len);
...
/* init auxiliary  node and functions */
	...
	/* [jiangfuqiang add] add hw info node */
	gt1x_init_hw_info_node();
