...
/* [liliwen start] change winfac values */
/* [liguanxiong edit] TP801 and TP802 use different calibration param */
#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
uint16_t winfac1 = 130;/*100;*/
uint16_t winfac2 = 250;/*80;*/
uint16_t winfac3 = 35;/*44;*/
#elif defined(CONFIG_TPLINK_PRODUCT_TP802)
uint16_t winfac1 = 100;
uint16_t winfac2 = 150;
uint16_t winfac3 = 40;
#else
uint16_t winfac1 = 100;
uint16_t winfac2 = 80;
...
/* [liguanxiong start] fix overflow cause lux_val unusual */
static uint32_t last_formula_value = 0;
#define MAX_FORMULA_VALUE	50000

static uint16_t lux_formula(uint16_t ch0_adc, uint16_t ch1_adc, uint8_t eqtn)
{
	uint32_t luxval = 0;
	uint32_t ch0_coeff = 0;
	uint32_t ch1_coeff = 0;
	int8_t ret;
	...
	if (eqtn == 1) {
		...
		if (ch0_adc * ch0_coeff < ch1_adc * ch1_coeff) {
			return last_formula_value;
		}
		else {
			luxval = ((ch0_adc * ch0_coeff) - (ch1_adc * ch1_coeff)) * win_fac / gain / als_int_fac / 10000;
	...
	if (luxval > MAX_FORMULA_VALUE) {
		luxval = MAX_FORMULA_VALUE;
	}
	last_formula_value = luxval;
	return luxval;
}
/* [liguanxiong end]*/
				...
				struct ltr559_data *ltr559)
{
	...
	/* [liguanxiong edit] change the default value, make sure the initial
           state can report to user space every time we enable the P-Sensor */
	input_report_abs(ltr559->ps_input_dev, ABS_DISTANCE, FAR_VAL + 1);
...
/* [liguanxiong start] avoid P-Sensor interrupt trigger frequently */
/* Report PS input event */
#define OBJECT_NEAR_VAL		1024
static void report_ps_input_event(struct ltr559_data *ltr559)
{
	...
	if (ps_grabData_stage == 1) {
		...
		/* [liguanxiong edit] change value 4 -> 6 */
		ntf_final *= 6;
		ntf_final /= 100;
		ntf_final += ps_ct_avg;
		ftn_final /= 10;
	...
	if (ps_grabData_stage == 2) {
		...
		/* [liliwen start] report NEAR to the user layer when PANEL ON in the packet */
		if ((adc_value >= ftn_final) || (adc_value < ntf_final)) {
			...
			if (adc_value > ftn_final) {
				...
				input_sync(ltr559->ps_input_dev);
				ret = set_ps_range(ntf_final, PS_MAX_MEASURE_VAL,
						LO_N_HI_LIMIT, ltr559);
				if (ret < 0) {
					dev_err(&ltr559->i2c_client->dev,
						"%s : PS thresholds setting Fail...\n", __func__);
			...
			/* the situation in pocket */
			if (adc_value <= ftn_final && adc_value >= OBJECT_NEAR_VAL) {
				input_report_abs(ltr559->ps_input_dev,
					ABS_DISTANCE, NEAR_VAL);
				input_sync(ltr559->ps_input_dev);
				ret = set_ps_range(ntf_final, PS_MAX_MEASURE_VAL,
			...
			if (adc_value < ntf_final && adc_value < OBJECT_NEAR_VAL) {
				input_report_abs(ltr559->ps_input_dev,
					ABS_DISTANCE, FAR_VAL);
				input_sync(ltr559->ps_input_dev);
				ret = set_ps_range(0, ftn_final,
						LO_N_HI_LIMIT, ltr559);
				if (ret < 0) {
					dev_err(&ltr559->i2c_client->dev,
						"%s : PS thresholds setting Fail...\n", __func__);
...
/* [liguanxiong end] avoid P-Sensor interrupt trigger frequently */
...
static int8_t als_enable_init(struct ltr559_data *ltr559)
{
	...
	/* [liguanxiong edit] change als_meas_rate 0x03 -> 0x08 (500ms -> 50ms) */
	rc = als_meas_rate_reg_setup(0x08, ltr559);
	if (rc < 0) {
		dev_err(&ltr559->i2c_client->dev,
...
/***********************************************************************/
static ssize_t ltr559_show_reg(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int i,len=0;
	uint8_t buffer[1];
...
static DEVICE_ATTR(reg, 0444, ltr559_show_reg, NULL);
/************************************************************************/
...
/* [liguanxiong start] Fix Bug(#1337) P-Sensor probe failed cause kernel panic(1/2) */
...
static int ltr559_enable_als_sensor(struct i2c_client *client, int val)
{
	...
	if (val == 1) {
		...
		if (data->enable_als_sensor == 0) {
			...
			/* [liuchangdong start] add dummy read*/
			buffer[0] = ltr559_ALS_CONTR;
			I2C_Read(buffer, 1);
			pr_debug("%s: ALS_CONTR 0x%x \n", __func__, buffer[0]);
			/* [liuchangdong end] */
		}
	} else {
		/* [liguanxiong start] Fix Bug #2814 */
		adc_value = read_als_adc_value(data);
		/* when disable als sensor, report lux value 1, if current lux value is 0 */
		if (adc_value == 0) {
			input_report_abs(data->als_input_dev, ABS_MISC, 1);
...
/* [liguanxiong start] create P/L-sensor on_board_info interface file (1/2) */
/*TODO: to identify ltr553 and ltr559, change this value*/
/* [liliwen edit] remove is_ltr553 */
//static int is_ltr553 = 0;

static ssize_t als_and_ps_board_info_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
/* [liliwen start] district LTR553/559 */
/* [liguanxiong edit] TP801 and TP802 use the same sensor type  */
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
			"LTR-559ALS-034", "LITE-ON", DRIVER_VERSION);
/* [liliwen end] */
}

static DEVICE_ATTR(light_sensor, S_IRUGO, als_and_ps_board_info_show, NULL);
/* [liguanxiong end] */
						...
						const struct i2c_device_id *id)
{
	...
	/* [liguanxiong start] create P/L-sensor on_board_info interface file (2/2) */
	/* The interface file will be seen in the user space as "/sys/kernel/light_sensor"
	if is created succesfully. */
	ret = sysfs_create_file(kernel_kobj, &dev_attr_light_sensor.attr);
	if (ret) {
		pr_err("%s:failed to create P/L-sensor on_board_info interface file\n", __func__);
	}
	/* [liguanxiong end] */
	return ret;
...
err_ltr559_setup:
/* [liguanxiong start] Fix Bug(#1337) P-Sensor probe failed cause kernel panic(1/2) */
	wake_lock_destroy(&(ltr559->ps_wake_lock));
/* [liguanxiong end] */
...
static void __exit ltr559_exit(void)
{
	...
	/* [liguanxiong start] remove P/L-sensor on_board_info interface file */
	sysfs_remove_file(kernel_kobj, &dev_attr_light_sensor.attr);
