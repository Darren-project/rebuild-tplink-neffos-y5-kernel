...
struct emmc_hw_info emmc_hw_info_list[] = {
	[SD9DS28K]    = { "SD9DS28K-8G",      "SanDisk"},
	[KMF720012M]  = { "KMF720012M-B214",  "Samsung"},
	[KMQ72000SM]  = { "KMQ72000SM-B316",  "Samsung"},
	[KMQ820013M]  = { "KMQ820013M-B419",  "Samsung"},

	[SUPPORT_MAX] = { "Unknow",  "Unknow"},
};

static ssize_t emmc_hw_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mmc_card *card = mmc_dev_to_card(dev);
	int ret;
	enum support_emmc index;

	for(index = 0; index < SUPPORT_MAX; index++) {
		if(!strcmp(emmc_card_name[index], card->cid.prod_name)) {
			break;
		}
	}
...
static DEVICE_ATTR(hw_info, S_IRUGO | S_IWUSR, emmc_hw_info_show, NULL );
