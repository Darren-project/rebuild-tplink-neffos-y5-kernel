 ...
 */
static int last_request_repower = 0;
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (err && cmd->retries && !mmc_card_removed(host->card)) {
		...
		if (!err) {
			host->enable_repower_sd = 1;
		}
		if (unlikely(err && host->cap_repower_sd)) {
			if (host->enable_repower_sd && host->ops && host->ops->repower_sd \
			    && host->card && mmc_card_present(host->card) \
			    && cmd->opcode != MMC_SEND_TUNING_BLOCK \
			    && cmd->opcode != MMC_SEND_TUNING_BLOCK_HS200 \
			    && cmd->opcode != MMC_SEND_TUNING_BLOCK_HS400) {
				host->ops->repower_sd(host, 1);
				last_request_repower = 1;
				host->enable_repower_sd = 0;
				pr_err("CMD op=%d, err=%d\n", opcode, err);
			}
		}
		else if (!err && host->cap_repower_sd && last_request_repower \
				&& host->card && mmc_card_present(host->card) \
				&& cmd->opcode != MMC_GO_IDLE_STATE ) {
			/* last requeset cause repower, we should cancel the repower delay work */
			last_request_repower = 0;
			if (host->ops && host->ops->repower_sd) {
				pr_err("cancel repower delaywork, op=%d, err=%d\n", opcode, err);
				host->ops->repower_sd(host, 0);
			}
		}
	}
}
