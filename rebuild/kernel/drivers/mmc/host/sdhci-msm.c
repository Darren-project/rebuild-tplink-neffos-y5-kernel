...
/* [liguanxiong edit] change 1000 -> 300 */
static int msecs_poweroff_sd = 300;
...
struct sdhci_msm_reg_data {
	...
	bool is_always_on;
	bool saved_is_always_on;
	/* is low power mode setting required for this regulator? */
	bool lpm_sup;
	bool set_voltage_sup;

	bool in_repower_process;
};
...
struct sdhci_msm_host {
	...
	bool use_cdclp533;
	struct device_attribute repower;
	struct delayed_work enable_sd_power_work;
	int repower_cnt;
};
		...
		struct sdhci_msm_reg_data **vreg_data, const char *vreg_name)
{
	...
	vreg->saved_is_always_on = vreg->is_always_on;
						...
						struct sdhci_host *host)
{
	...
	if (of_get_property(np, "qcom,cap-repower-sd", NULL))
		pdata->cap_repower_sd = true;
...
static int sdhci_msm_vreg_enable(struct sdhci_msm_reg_data *vreg)
{
	...
	/* [yanghuaying] disable vdd may trigger sdhci_msm_pwr_irq, which will call
	 * sdhci_msm_vreg_enable, this make disble action has no mean, and may make
	 * vdd's disable time not enough, so not enable regulator when in repower_process
	 */
	if(vreg->in_repower_process)
		return 0;
	...
	if (!vreg->is_enabled) {
		...
		/* [yanghuaying] move next sentence in to balance regulator_en(dis)able */
...
static int sdhci_msm_repower_sd(struct sdhci_host *host, int enable)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
...
//	bool saved_is_always_on;
	...
	if (enable) {
		msm_host->repower_cnt++;
	}
	else {
		msm_host->repower_cnt--;
	}
/*	if (!msm_host->pdata->vreg_data) {
		return 0;
	}
	curr_vdd_reg = msm_host->pdata->vreg_data->vdd_data;
	saved_is_always_on = curr_vdd_reg->is_always_on;
	curr_vdd_reg->is_always_on = false;
	sdhci_msm_vreg_disable(curr_vdd_reg);
	curr_vdd_reg->is_always_on = saved_is_always_on;*/
	if (enable) {
		queue_delayed_work(system_nrt_wq, &msm_host->enable_sd_power_work, \
				msecs_to_jiffies(100));
	}
	else {
		cancel_delayed_work(&msm_host->enable_sd_power_work);
	}

	return 0;
}

static void sdhci_msm_poweron_sd(struct work_struct *work)
{
	struct sdhci_msm_host *msm_host;
	struct sdhci_msm_reg_data *curr_vdd_reg;

	msm_host = container_of(work, struct sdhci_msm_host, enable_sd_power_work.work);
        if (!msm_host->pdata->vreg_data) {
		return ;
        }

	pr_err("cnt:%d,msecs:%d\n", msm_host->repower_cnt, msecs_poweroff_sd);
	curr_vdd_reg = msm_host->pdata->vreg_data->vdd_data;
	curr_vdd_reg->is_always_on = false;
	curr_vdd_reg->in_repower_process = true;
	sdhci_msm_vreg_disable(curr_vdd_reg);
	curr_vdd_reg->is_always_on = curr_vdd_reg->saved_is_always_on;
	msleep(msecs_poweroff_sd);
	curr_vdd_reg->in_repower_process = false;
        sdhci_msm_vreg_enable(curr_vdd_reg);
}

static ssize_t repower_store(struct device *dev, struct device_attribute
				*attr, const char *buf, size_t count)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	u32 tmp;
	...
	if (!kstrtou32(buf, 0, &tmp)) {
		msecs_poweroff_sd = tmp;
	}
	sdhci_msm_repower_sd(host, 1);
	return count;
}

static ssize_t repower_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	...
	return snprintf(buf, PAGE_SIZE, "cnt:%d,msecs:%d\n", msm_host->repower_cnt, msecs_poweroff_sd);
}
...
static struct sdhci_ops sdhci_msm_ops = {
	...
	.repower_sd = sdhci_msm_repower_sd,
};
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (msm_host->pdata->cap_repower_sd) {
		msm_host->mmc->cap_repower_sd = 1;
		INIT_DELAYED_WORK(&msm_host->enable_sd_power_work, sdhci_msm_poweron_sd);
	}
	...
	if (msm_host->pdata->cap_repower_sd) {
		msm_host->repower.show = repower_show;
		msm_host->repower.store = repower_store;
		sysfs_attr_init(&msm_host->repower.attr);
		msm_host->repower.attr.name = "repower";
		msm_host->repower.attr.mode = S_IRUGO | S_IWUGO;
		device_create_file(&pdev->dev, &msm_host->repower);
	}
...
static int sdhci_msm_remove(struct platform_device *pdev)
{
	...
	if (msm_host->pdata->cap_repower_sd) {
		device_remove_file(&pdev->dev, &msm_host->repower);
		cancel_delayed_work_sync(&msm_host->enable_sd_power_work);
	}
