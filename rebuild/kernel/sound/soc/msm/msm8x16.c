...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	...
	gpio_set_value(pdata->spk_ext_pa_gpio, enable);
...
static const char *card_param_ver = NULL;

static ssize_t
card_param_ver_show_attr(struct device *dev,
		     struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "param_version:\t%s\n", card_param_ver ? card_param_ver : "");
}

static struct device_attribute card_param_ver_attrs =
	__ATTR(param_ver, S_IRUGO, card_param_ver_show_attr, NULL);
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	const char *param_ver = "qcom,msm-param-ver";
	const char *ptr = NULL;
	...
	ret = of_property_read_string(pdev->dev.of_node, param_ver, &card_param_ver);
	if (ret) {
		card_param_ver = "";
	}
	...
	ret = device_create_file(card->snd_card->card_dev, &card_param_ver_attrs);
	if (ret < 0) {
		dev_err(&pdev->dev, "parameter version device file create failed (%d)\n",
			ret);
	}
