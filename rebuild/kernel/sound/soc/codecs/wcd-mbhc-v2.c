...
correct_plug_type:
	 ...
	 */
	if (!wrk_complete && plug_type == MBHC_PLUG_TYPE_HEADSET && !reported_headset) {
...
static void wcd_mbhc_detect_plug_type(struct wcd_mbhc *mbhc)
{
	...
	if(current_ext_spk_pa_state) {
		pr_info("%s:Close FSM one time when Spk on\n",__func__);
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN, 0);
	}
