...
#define NUM_EXT_PA_PULSE_PATTERN_3	2
#define NUM_EXT_PA_PULSE_PATTERN_2	1
#define NUM_EXT_PA_PULSE_PATTERN_1	0
#define DELAY_USEC_EXT_PA_PULSE 	2
...
#define MICBIAS_DEFAULT_VAL 2000000
...
static void enable_ext_spk_gpio(struct work_struct *work)
{
	int i = 0;
	int pattern = 0;

#if defined(CONFIG_TPLINK_PRODUCT_TP801) || defined(CONFIG_TPLINK_PRODUCT_TP803)
	pattern = NUM_EXT_PA_PULSE_PATTERN_1;
#elif defined(CONFIG_TPLINK_PRODUCT_TP802)
	pattern = NUM_EXT_PA_PULSE_PATTERN_2;
#else
 #error Ext-PA pulse pattern must be specified.
#endif

	gpio_direction_output(ext_spk_pa_gpio, 0);

	//Output pulse according to external PA pattern
	while (i < pattern) {
		gpio_set_value(ext_spk_pa_gpio, 1);
		udelay(DELAY_USEC_EXT_PA_PULSE);
	...
	gpio_set_value(ext_spk_pa_gpio, 1);
	pr_info("enable_ext_spk_gpio ... set gpio on ...\n");
	current_ext_spk_pa_state = true;
}

static DECLARE_DELAYED_WORK(ext_spk_work, enable_ext_spk_gpio);

bool using_spk_ext_amp = false;
	...
	switch (ucontrol->value.integer.value[0]) {
		case 0:
			if (gpio_is_valid(ext_spk_pa_gpio)) {
				...
				using_spk_ext_amp = false;
			}
			break;
		case 1:
			if (gpio_is_valid(ext_spk_pa_gpio)) {
				using_spk_ext_amp = true;
				schedule_delayed_work(&ext_spk_work, msecs_to_jiffies(70));
			}
			break;
		default:
			return -EINVAL;
...
static const struct snd_kcontrol_new msm8x16_wcd_snd_controls[] = {
	...
	SOC_ENUM_EXT("Speaker Ext Amp", msm8x16_wcd_ext_spk_ctl_enum[0],
...
static const struct snd_kcontrol_new ext_spk_pa_mux =
	SOC_DAPM_ENUM_VIRT("Speaker Ext Mux", msm8x16_wcd_ext_spk_ctl_enum[0]);
...
static const struct snd_soc_dapm_route audio_map[] = {
	...
	{"Speaker Ext PA", NULL, "Speaker Ext"},
	{"Speaker Ext", "ENABLE", "HPHR PA"},
...
int msm8x16_trigger(struct snd_pcm_substream * subs, int cmd,struct snd_soc_dai *soc_dai)
{
	if(current_ext_spk_pa_state && !cmd && !using_spk_ext_amp) {
		//pr_info("cmd is %d close spk_pa .....\n",cmd);
		//gpio_direction_output(ext_spk_pa_gpio, 0);
	}
	return 0;
}
...
static int spk_ext_pa_enable(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
...
#elif defined(CONFIG_TPLINK_PRODUCT_TP802)
	pattern = NUM_EXT_PA_PULSE_PATTERN_2;
#elif defined(CONFIG_TPLINK_PRODUCT_TP803)
	pattern = NUM_EXT_PA_PULSE_PATTERN_2;
#else
 #error Ext-PA pulse pattern must be specified.
#endif
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		dev_info(w->codec->dev,"%s: enable external speaker PA\n", __func__);
		...
		break;
	case SND_SOC_DAPM_PRE_PMD:
		dev_info(w->codec->dev,"%s: disable external speaker PA\n", __func__);
		//Trigger alread close pa;
		gpio_set_value(ext_spk_pa_gpio, 0);
		current_ext_spk_pa_state = false;
		break;
	}
...
static const struct snd_soc_dapm_widget msm8x16_wcd_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SPK("Speaker Ext PA", spk_ext_pa_enable),
	...
	SND_SOC_DAPM_VIRT_MUX("Speaker Ext", SND_SOC_NOPM, 0, 0,&ext_spk_pa_mux),
